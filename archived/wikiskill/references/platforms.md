...
WikiSkill 的核心是 `.wiki/` 目录结构和 Markdown 文件格式，与具体 Agent 平台无关。本文档说明如何在不同平台上使用。
...
`.workbuddy/skills/wikiskill/` 是 **WorkBuddy 的私有目录**。它只在装了 WorkBuddy
...
| 用本 skill 的实际解压路径 | 如 `D:\tools\wikiskill\scripts\wiki_init.py` |
...
- 建议在项目 README 中说明 WikiSkill 的使用方式
...
**推荐**：优先使用 WorkBuddy / CodeBuddy，体验最完整。
